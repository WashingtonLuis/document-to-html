	...
	try {
		...
		if (document.getElementById("exerciciosFundamental1").checked) {
			textareaValue = textareaValue
				...
				.replace(/<span[^>]*class="[^"]*\bCharOverride-\b(?:5|6)\b[^"]*"[^>]*>([\s\S]*?)<\/span>/gi, "<span class='text-warning'><b>$1</b></span> ")
				.replace(/<span[^>]*class="[^"]*\bCharOverride-\b(?:26|27|28)\b[^"]*"[^>]*>([\s\S]*?)<\/span>/gi, "<span class='text-danger'><b>$1</b></span>")
				...
				.replace(/<span[^>]*class="[^"]*\bBold\b[^"]*"[^>]*>([\s\S]*?)<\/span>/gi, "<b>$1</b> ")
